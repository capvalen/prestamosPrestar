...
if($totalRow==0){
	...
	while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
	{
		...
		<tr data-id="<?= $row['idCaja']; ?>" data-activo="<?= $row['cajaActivo']; ?>">
			...
			<td class='mayuscula tpIdDescripcion'><?= $row['tipoDescripcion'];?></td> <td><i class="icofont icofont-bubble-right"></i> <em class="emRegistra"><?= $row['usuNick'];?></em></td> <td>S/ <span class='spanCantv3'><?= number_format(round($row['pagoMonto'],1),2);?></span></td> <td class='mayuscula tdMoneda' data-id="<?= $row['cajaMoneda'];?>" ><?= $row['moneDescripcion'];?></td> <td class='mayuscula tdObservacion'><?= $row['cajaObservacion'];?></td> <td><span class="sr-only fechaPagov3"><?= $row['cajaFecha']; ?></span> <?= $boton; ?></td> </tr>
		<?php 
		if($totalRow==$i){
